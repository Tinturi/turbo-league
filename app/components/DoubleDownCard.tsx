...
type Status = {
  ...
  total: number;
  used: number;
  pending: { id: number; activatedAt: string } | null;
  remaining: number;
};
...
export default function DoubleDownCard({ playerId }: { playerId: number }) {
  ...
  async function activate() {
    ...
    try {
      ...
      setStatus(data.status);
      setMessage(data.message || "Double Down активирован");
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
    } finally {
      setActivating(false);
    ...
    <section style={{ marginBottom: 24, padding: 22, border: "1px solid rgba(233,184,75,.36)", borderRadius: 18, background: "linear-gradient(135deg, rgba(62,31,6,.78), rgba(16,20,29,.96) 52%, rgba(10,14,21,.96))", boxShadow: "0 18px 45px rgba(0,0,0,.24)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 18, alignItems: "flex-start", flexWrap: "wrap" }}>
        <div style={{ minWidth: 240, flex: "1 1 380px" }}>
          ...
          <p className="muted" style={{ margin: 0, lineHeight: 1.5, maxWidth: 690 }}>
            Нажимай после начала Turbo-матча и оценки пиков. Система привяжет DD к игре, которая началась не более 10 минут назад. Победа и поражение обе считаются x2.
          </p>
        </div>
      ...
      <div style={{ marginTop: 18, display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        <button
          type="button"
          onClick={activate}
          disabled={loading || activating || Boolean(status?.pending) || (status?.remaining ?? 0) <= 0}
          style={{ border: "1px solid rgba(255,190,73,.62)", borderRadius: 13, padding: "13px 18px", background: status?.pending ? "rgba(255,156,45,.15)" : "#e9b84b", color: status?.pending ? "#f4bf66" : "#17120a", fontWeight: 900, fontSize: 14, cursor: loading || activating || status?.pending || (status?.remaining ?? 0) <= 0 ? "default" : "pointer", opacity: loading || activating || (status?.remaining ?? 0) <= 0 ? .66 : 1 }}
        >
