...
async function getStatus(playerId: number) {
  ...
  const [{ data: bonuses, error: bonusError }, { data: activations, error: activationError }] = await Promise.all([
    supabaseAdmin
      ...
      .select("id,source_match_id,reason")
      .eq("player_id", playerId)
      .eq("week_start", weekStartIso),
    supabaseAdmin
      .from("double_down_activations")
      .select("id,activated_at,status,match_id")
      .eq("player_id", playerId)
      .eq("week_start", weekStartIso)
  ...
  return {
    ...
    bonuses: bonusCount,
    total,
    used: activeRows.filter((row) => row.status === "used").length,
...
export async function GET(req: NextRequest) {
  ...
  try {
    return NextResponse.json({ ok: true, status: await getStatus(playerId) });
  } catch (error) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    body = (await req.json()) as { playerId?: number };
  } catch {
  ...
  if (!Number.isInteger(playerId) || playerId <= 0) {
    return NextResponse.json({ ok: false, error: "Некорректный игрок" }, { status: 400 });
  }
  ...
  try {
    ...
    if (status.pending) {
      return NextResponse.json({ ok: false, error: "Double Down уже активирован и ждёт текущий матч", status }, { status: 409 });
    }
    if (status.remaining <= 0) {
      return NextResponse.json({ ok: false, error: "Double Down на этой неделе закончились", status }, { status: 409 });
    }

    const now = new Date().toISOString();
    const { error } = await supabaseAdmin.from("double_down_activations").insert({
      player_id: playerId,
      activated_at: now,
