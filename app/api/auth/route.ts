...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ ok: true, release: "accounts-20260909-v1", account, players: (players ?? []).filter(row => !taken.has(Number(row.id))) }, { headers: privateHeaders });
  } catch {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return response;
  } catch { return NextResponse.json({ ok: false, error: "Сервис аккаунтов временно недоступен. Попробуйте позже." }, { status: 503 }); }
}
