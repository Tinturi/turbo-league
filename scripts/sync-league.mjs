...
async function cleanupSeason3Data() {
  const { data: oldMatches, error: matchError } = await supabaseAdmin.from("matches").delete().gte("start_time", OLD_SEASON3_START_ISO).lt("start_time", SEASON_START_ISO).select("match_id");
  if (matchError) throw new Error(`Season 3 cleanup matches: ${matchError.message}`);
  const { error: ddError } = await supabaseAdmin.from("double_down_activations").delete().lt("activated_at", SEASON_START_ISO);
  if (ddError) throw new Error(`Season 3 cleanup DD activations: ${ddError.message}`);
  const { error: bonusError } = await supabaseAdmin.from("double_down_bonuses").delete().lt("created_at", SEASON_START_ISO);
  if (bonusError) throw new Error(`Season 3 cleanup DD bonuses: ${bonusError.message}`);
  const removed = oldMatches?.length ?? 0;
  if (removed > 0) {
    const { error: resetError } = await supabaseAdmin.from("players").update({ rating: 0, wins: 0, losses: 0, tracking_from: SEASON_START_ISO }).eq("active", true);
...
async function refundIgnoredDoubleDowns(playerId, ids) { if (!ids.length) return; const { error } = await supabaseAdmin.from("double_down_activations").update({ status: "expired" }).eq("player_id", playerId).eq("status", "used").in("match_id", ids); if (error) throw new Error(`Refund ignored Double Down: ${error.message}`); }
async function removeIgnoredBonuses(playerId, ids) { if (!ids.length) return; const { error } = await supabaseAdmin.from("double_down_bonuses").delete().eq("player_id", playerId).in("source_match_id", ids); if (error) throw new Error(`Remove ignored DD bonuses: ${error.message}`); }
async function removeIneligibleStoredMatches(playerId, eligibleMatches) {
  const eligibleIds = new Set(eligibleMatches.map((m) => Number(m.match_id)));
  const { data: stored, error: lookupError } = await supabaseAdmin.from("matches").select("match_id").eq("player_id", playerId).gte("start_time", SEASON_START_ISO);
  if (lookupError) throw new Error(`Stored Season 4 matches lookup: ${lookupError.message}`);
  const removeIds = (stored ?? []).map((r) => Number(r.match_id)).filter((id) => !eligibleIds.has(id));
  if (!removeIds.length) return 0;
  await refundIgnoredDoubleDowns(playerId, removeIds); await removeIgnoredBonuses(playerId, removeIds);
  const { data, error } = await supabaseAdmin.from("matches").delete().eq("player_id", playerId).in("match_id", removeIds).select("match_id");
  if (error) throw new Error(`Remove ineligible Season 4 matches: ${error.message}`); return data?.length ?? 0;
}

async function attachDoubleDowns(playerId, allTurboMatches, eligibleMatches) {
  const { data: activationRows, error } = await supabaseAdmin.from("double_down_activations").select("id,activated_at,status,match_id,week_start").eq("player_id", playerId).gte("activated_at", SEASON_START_ISO).order("activated_at", { ascending: true });
  if (error) throw new Error(`Double Down lookup: ${error.message}`);
  const eligibleIds = new Set(eligibleMatches.map((m) => Number(m.match_id))), activations = activationRows ?? [];
  const usedMatchIds = new Set(activations.filter((r) => r.status === "used" && r.match_id != null && eligibleIds.has(Number(r.match_id))).map((r) => Number(r.match_id)));
  for (const activation of activations.filter((r) => r.status === "pending")) {
    const activatedMs = new Date(activation.activated_at).getTime();
    const playedMatch = allTurboMatches.find((m) => { const s = Number(m.start_time ?? 0) * 1000, d = Math.max(0, Number(m.duration ?? 0)) * 1000; return s <= activatedMs && activatedMs <= s + d; });
    if (playedMatch) {
      const matchId = Number(playedMatch.match_id), startMs = Number(playedMatch.start_time ?? 0) * 1000, inTime = activatedMs <= startMs + DOUBLE_DOWN_WINDOW_MS, eligible = eligibleIds.has(matchId), available = !usedMatchIds.has(matchId);
      if (inTime && eligible && available) { const { error: e } = await supabaseAdmin.from("double_down_activations").update({ status: "used", match_id: matchId }).eq("id", activation.id).eq("status", "pending"); if (e) throw new Error(`Double Down attach ${activation.id}: ${e.message}`); usedMatchIds.add(matchId); console.log(`  🔥 Double Down -> match ${matchId}`); }
      else { const reason = !inTime ? "нажат позже 10 минут" : !eligible ? "матч не идёт в зачёт Season 4" : "матч уже использован для DD"; const { error: e } = await supabaseAdmin.from("double_down_activations").update({ status: "expired" }).eq("id", activation.id).eq("status", "pending"); if (e) throw new Error(`Double Down refund ${activation.id}: ${e.message}`); console.log(`  ↩ Double Down ${activation.id} refunded: ${reason}, match ${matchId}`); }
      continue;
    }
    if (Date.now() - activatedMs > DOUBLE_DOWN_PENDING_TTL_MS) { const { error: e } = await supabaseAdmin.from("double_down_activations").update({ status: "expired" }).eq("id", activation.id).eq("status", "pending"); if (e) throw new Error(`Double Down expire ${activation.id}: ${e.message}`); console.log(`  ↩ Double Down ${activation.id} expired by fallback TTL and refunded`); }
  }
  return usedMatchIds;
}

async function grantLossStreakBonuses(playerId, eligibleMatches) {
  const weekStart = getWeekStart(), weekStartUnix = Math.floor(weekStart.getTime() / 1000), weeklyMatches = eligibleMatches.filter((m) => Number(m.start_time ?? 0) >= weekStartUnix);
  let consecutiveLosses = 0, granted = 0;
  for (const match of weeklyMatches) { if (didPlayerWin(match)) { consecutiveLosses = 0; continue; } consecutiveLosses += 1; if (consecutiveLosses % 3 !== 0) continue; const { error } = await supabaseAdmin.from("double_down_bonuses").upsert({ player_id: playerId, week_start: weekStart.toISOString(), source_match_id: Number(match.match_id), reason: "three_losses" }, { onConflict: "player_id,source_match_id", ignoreDuplicates: true }); if (error) throw new Error(`Double Down bonus ${match.match_id}: ${error.message}`); granted += 1; }
...
async function syncPlayer(player) {
  ...
  const allTurboMatches = rawMatches.filter((m) => Number(m.game_mode) === 23).filter((m) => Number(m.start_time ?? 0) >= SEASON_START_UNIX).sort((a,b) => Number(a.start_time ?? 0)-Number(b.start_time ?? 0));
  const { eligible: eligibleMatches, ignored } = filterMatchesByWeeklyHeroes(allTurboMatches, selections), ignoredMatchIds = ignored.map(({match}) => Number(match.match_id));
  ...
  if (ignored.length) console.log(`  🚫 Не в зачёте: ${ignored.map(({match,reason}) => `${match.match_id}(${reason})`).join(", ")}`);
  const doubleDownMatchIds = await attachDoubleDowns(player.id, allTurboMatches, eligibleMatches);
  let currentRating=START_RATING,wins=0,losses=0,added=0,repaired=0;
  ...
  const {bonusMilestones}=await grantLossStreakBonuses(player.id,eligibleMatches); const{error:playerUpdateError}=await supabaseAdmin.from("players").update({rating:currentRating,wins,losses,tracking_from:SEASON_START_ISO}).eq("id",player.id);if(playerUpdateError)throw new Error(`Player update: ${playerUpdateError.message}`);
